//+------------------------------------------------------------------+
//|                             Hedge_Multiple_Volume_Max_Profit.mq5 |
//|                                                           minhnd |
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
...
input double TPMoveThreshold = 0.3;
...
bool IsTouchTPBuy = false;
bool IsTouchTPSell = false;
...
void OnTick()
{
    ...
    if (PositionsTotal() > 0 && OrdersTotal() == 0 && GetMaxVolumePosition() < MaxVolume)
    {
        ...
        if (Type == 0)
        {
            ...
            {
                trade.BuyStop(volume, BuyPrice, _Symbol, 0, 0);
                Print("***[EA]*** Place BUY Order, volume: ", volume);
            }
            else
            {
                trade.SellStop(volume, SellPrice, _Symbol, 0, 0);
    ...
    if (TakeProfit() && PositionsTotal() > 0)
    {
...
void Init()
{
    ...
    if (Type == 0)
    {
        ...
        trade.Buy(InitialVolume, _Symbol, BuyPrice, 0, 0);
    ...
    else if (Type == 1)
    {
        ...
        TPBuyPrice = NormalizeDouble(BuyPrice + (TPInPip + SpreadInPip) * SymbolPoint, _Digits);
        TPSellPrice = NormalizeDouble(SellPrice - TPInPip * SymbolPoint, _Digits);
        trade.Sell(InitialVolume, _Symbol, SellPrice, 0, 0);
...
void CheckTakeProfit()
{
    ...
    if (TPSellPrice != 0.0 && currentBidPrice < TPSellPrice - TPMoveThreshold)
    {
        IsTouchTPSell = true;
        TPSellPrice = currentBidPrice + TPMoveThreshold;
        Print("***[EA]*** TPSellPrice moved to: ", TPSellPrice);
    }
    
    double currentAskPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if (TPBuyPrice != 0.0 && currentAskPrice > TPBuyPrice + TPMoveThreshold)
    {
        IsTouchTPBuy = true;
        TPBuyPrice = currentAskPrice - TPMoveThreshold;
        Print("***[EA]*** TPBuyPrice moved to: ", TPBuyPrice);
    }
}
...
bool TakeProfit()
{
    double currentBidPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    double currentAskPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if ((IsTouchTPBuy && currentAskPrice < TPBuyPrice) || (IsTouchTPSell && currentBidPrice > TPSellPrice))
        return true;
    return false;
}
